...
[AddComponentMenu("FX/Water/Refract and Reflect")]
public class WaterEffect : MonoBehaviour
{
    public enum WaterMode
    {
        SolidColor = 0,   //WATER_SOLID
        Reflective = 1,   //WATER_REFLECTION
        Refractive = 2,   //WATER_REFRACTION
    };
    ...
    void SetRenderTexture(Renderer[] childRenderers, string name, RenderTexture texture)
    {
        for(int i=0;i<childRenderers.Length;++i)
        {
            if(childRenderers[i].enabled && childRenderers[i].sharedMaterial)
            {
                childRenderers[i].sharedMaterial.SetTexture(name, texture);
            }
        }
    }
    ...
    void OnDisable()
    {
        ...
        {
            Debug.Log(" camara:" + kvp.Value + " g:" + kvp.Value.gameObject);
    ...
    WaterMode FindHardwareWaterSupport()
    {
        ...
        {
            return WaterMode.SolidColor;
        }

        Material mat = GetComponentInChildren<Renderer>().sharedMaterial;
        if (!mat)
        {
            return WaterMode.SolidColor;
        }
